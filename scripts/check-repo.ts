...
function checkPolicy() {
  ...
  if (policy.productName !== 'TurboWarp-Voice-Chat') {
...
function checkPackageMetadata() {
  ...
  if (packageMetadata.repository?.url !== 'git+https://github.com/kubohiroya/turbowarp-voice-chat.git') {
    errors.push('package.json repository.url must point to the current repository');
  }
  if (packageMetadata.bugs?.url !== 'https://github.com/kubohiroya/turbowarp-voice-chat/issues') {
...
function checkReadmes() {
  ...
  if (!readmeJa.startsWith('# TurboWarp-Voice-Chat\n')) {
...
async function checkPackContents() {
  ...
  if (!files.has('dist/turbowarp-voice-chat.js')) {
